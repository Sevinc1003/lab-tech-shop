...
export default function AdBanner() {
  ...
  useEffect(() => {
    // Komponent brauzerdə mount olundu
    setIsMounted(true);
    
    // localStorage-ı yoxlayırıq
    const premiumStatus = localStorage.getItem('isPremium') === 'true';
    setIsPremium(premiumStatus);

    // Premium statusu dəyişəndə (ödəniş səhifəsində) reklamları anında gizlətmək üçün event
    const handlePremiumChange = () => {
      setIsPremium(localStorage.getItem('isPremium') === 'true');
    };

    window.addEventListener('premiumChange', handlePremiumChange);
    return () => window.removeEventListener('premiumChange', handlePremiumChange);
  }, []);

  // Server renderi və ilk hidratasiya zamanı uyğunsuzluq olmasın deyə boş qaytarırıq
